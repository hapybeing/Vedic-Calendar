...
export default function TodayPanel({ today }) {
  const color = today.score > 70 ? '#34d399' : today.score > 50 ? '#d4af37' : '#f87171';
  const c = 2 * Math.PI * 32;
  const offset = c - (today.score / 100) * c;
  ...
  return (
    <motion.section {...fadeUp} className="rounded-[30px] border border-white/10 bg-panel/70 p-7 shadow-glass backdrop-blur-2xl">
      ...
      <div className="mt-4 flex items-center gap-5">
        <svg width="78" height="78" className="shrink-0"><circle cx="39" cy="39" r="32" stroke="#2a2a2a" strokeWidth="8" fill="none" /><circle cx="39" cy="39" r="32" stroke={color} strokeWidth="8" strokeLinecap="round" fill="none" strokeDasharray={c} strokeDashoffset={offset} transform="rotate(-90 39 39)" /></svg>
        <div><p className="font-display text-3xl">{today.score}</p><p className="text-sm text-zinc-400">{today.label}</p></div>
      </div>
